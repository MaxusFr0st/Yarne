...
public static partial class UploadsToR2Migration
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (map.Count == 0)
        {
            logger.LogInformation("Uploads→R2 migration: nothing to rewrite");
            return report;
        }

        // 3. Repoint the database. Atomic, so a failure mid-way cannot leave half the gallery
        //    on one host and half on the other.
        if (dryRun)
        {
        ...
        logger.LogInformation(
            "Uploads→R2 migration complete: {Uploaded} uploaded, {Existing} already present, {Missing} missing on disk, {Failed} failed",
