...
public class FinishedGoodsLot
{
    ...
    /// <summary>
    /// Legacy counter from when finished units could be pushed into a storefront per-variant
    /// stock pool. The storefront no longer tracks pooled stock (made-to-order model), so
    /// nothing writes this anymore — it stays 0 for lots produced under the current model.
    /// Kept only so historical rows/values are not lost.
    /// </summary>
