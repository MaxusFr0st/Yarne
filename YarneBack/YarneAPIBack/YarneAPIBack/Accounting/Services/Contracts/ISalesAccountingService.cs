...
public interface ISalesAccountingService
{
    ...
    /// locks/decrements <c>FinishedGoodsInventory.QuantityOnHand</c>, so the FIFO ledger and the
    /// pooled counter move together. Throws <see cref="AccountingBusinessException"/> if not enough
    /// lot stock remains.
    /// </summary>
    ...
    /// <summary>
    /// Self-heal: resets <c>FinishedGoodsInventory.QuantityOnHand</c> to the sum of
    /// <c>FinishedGoodsLot.QuantityRemaining</c> (non-void lots) for every product that has a
    /// finished-goods ledger. Use when the pooled counter has drifted from the lot-authoritative
    /// ledger. Returns the number of rows adjusted.
    /// </summary>
