...
public sealed class SalesAccountingService : ISalesAccountingService
{
    ...
    /// <summary>
    /// Self-heal for pooled counter drift: recomputes <c>FinishedGoodsInventory.QuantityOnHand</c>
    /// from Σ non-void <c>FinishedGoodsLot.QuantityRemaining</c> for every product that has a
    /// finished-goods ledger row or lot. The lot ledger is the source of truth (it's what
    /// void-production and FIFO costing key off), so this repairs any pooled counter that has
    /// drifted from it — e.g. from data fixed up directly in the database.
    /// </summary>
